			...
			for _, buffering := range []bool{false, true} {
				t.Run(fmt.Sprintf("%s/lite=%s/buffering=%t", transport, lite, buffering), func(t *testing.T) {
					...
					if transport == "websocket" {
						...
						// This proxy pins the session header to the client's own
						// spelling on every path — hyphenated and lowercase, which the
						// real handshake sends — so there is no synthesized alias here to
						// tell native traffic from translated traffic apart. What has to
						// hold is that the cloaking alias never reaches the wire; read back
						// leniently, since Go's http server normalises incoming keys.
						if got := headerValueCaseInsensitive(upstreamHeaders, "session_id"); got != "" {
							t.Errorf("cloaking session_id alias reached the upstream: %q", got)
						}
						t.Logf("upstream session id: %q (native = %t)",
							headerValueCaseInsensitive(upstreamHeaders, "session-id"), native)
					}
